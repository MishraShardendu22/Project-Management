        ...
        {filteredTasks && filteredTasks.length > 0 ? (
          <ul className="space-y-4">
            {filteredTasks.map((task: any) => {
              const sanitizedDescription = DOMPurify.sanitize(task.description, { ALLOWED_TAGS: [] });
              const snippet =
                sanitizedDescription.length > 100 ? sanitizedDescription.substring(0, 100) + "..." : sanitizedDescription;
              return (
                <li key={task.id} className="border p-4 rounded shadow-sm">
                  ...
                  <div className="text-sm text-gray-700 mt-2">
                    ...
                    {sanitizedDescription.length > 100 && (
                      <Button onClick={() => setModalTask(task)} variant="link" className="mt-1">
                        View Description
                      </Button>
                    )}
                  </div>
                  <p className="text-sm text-gray-500 mt-2">Due Date: {task.dueDate}</p>
                  <p className="text-sm text-gray-500">Priority: {task.priority === 1 ? "Very High" : task.priority === 2 ? "High" : "Moderate"}</p>
                  <div className="flex gap-2 mt-4">
                    <Button onClick={() => { setEditingTask(task); setEditForm({ title: task.title, description: sanitizedDescription, dueDate: task.dueDate, priority: task.priority }); }}>Edit</Button>
                    <Button onClick={() => handleDeleteTask(task.id.toString())} variant="destructive">Delete</Button>
                  </div>
                </li>
              );
            })}
